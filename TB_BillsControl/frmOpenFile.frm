...
Private Sub cmbStyleNumber_Change()
    MsgBox cmbStyleNumber.Text
End Sub
...
On Error GoTo Err
    ...
    strSQL = strSQL & vbCrLf & "(select distinct t1.FStyleNumber,t1.FColorEx as FColorCode,t1.FColorDesc,t1.FSizeEx from t_icitem t1"
'    strSQL = strSQL & vbCrLf & "left join t_SubMessage t2 on t1.FColorExID = t2.FInterID where t2.FTypeID=11600"
    strSQL = strSQL & vbCrLf & "where isnull(t1.FStyleNumber,'')<>'' and isnull(t1.FColorEx,'')<>'' and isnull(t1.FSizeEx,'')<>'' ) tt"
    Set rs = ExecuteSql(strSQL)
...
'    strSQL = strSQL & vbCrLf & "left join t_SubMessage t2 on t1.FColorExID = t2.FInterID where t2.FTypeID=11600"
    ...
    strSQL = "select distinct t1.FItemID,t1.FStyleNumber,t1.FColorEx as FColorCode,t1.FColorDesc,t1.FSizeEx,t1.FNumber,0 as Flag from t_icitem t1"
'    strSQL = strSQL & vbCrLf & "left join t_SubMessage t2 on t1.FColorExID = t2.FInterID where t2.FTypeID=11600"
    strSQL = strSQL & vbCrLf & "where isnull(t1.FStyleNumber,'')<>'' and isnull(t1.FColorEx,'')<>'' and isnull(t1.FSizeEx,'')<>''"
    Set rs = ExecuteSql(strSQL)
